...
createItem(item){
    ...
    const total = createElement('form', {className: 'frm invise', 'data-id': item.id}, first, second, third, fourth);
    return total
}

createItem2(item){
    const first = createElement('h3', {className: 'solve'}, item.title[0]);
    const second = createElement('p', {className: 'solve2'}, item.title[1])
    const sums = createElement('div', {className: 'somediv', 'data-id': item.id}, first, second);
    return sums
}
